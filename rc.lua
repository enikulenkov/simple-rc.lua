...
-- https://github.com/Mic92/utils/blob/master/cal.lua
...
function volume (mode, widget)
  if mode == "update" then
    ...
    if volume ~= nil then
      ...
      widget.text = widget:set_value(tonumber(volume))
    else
      widget:set_background_color("#999933")
    end
  elseif mode == "up" then
...
terminal = "urxvt -fn \"xft:Terminus:pixelsize=14,style=regular\""
...
globalkeys = awful.util.table.join(
	...
	-- Lock screen
	awful.key({ modkey, "Control" }, "s", function() awful.util.spawn("dm-tool switch-to-greeter") end),

  -- Fix bug with black screen (https://bbs.archlinux.org/viewtopic.php?id=202810)
	awful.key({ modkey, "Control" }, "x", function() awful.util.spawn("xset dpms force off") end)
)
...
if os.getenv("AWESOME_FROM_VNC") ~= nil then
keycode0 = 17
else
keycode0 = 9
end
for i = 1, keynumber do
    globalkeys = awful.util.table.join(globalkeys,
        ...
        awful.key({ modkey, "Control" }, "#" .. i + keycode0,
                  function ()
                      local screen = mouse.screen
                      if tags[screen][i] then
        ...
        awful.key({ modkey, "Shift" }, "#" .. i + keycode0,
                  function ()
                      if client.focus then
        ...
        awful.key({ modkey, "Control", "Shift" }, "#" .. i + keycode0,
                  function ()
                      if client.focus then
                          local tag = awful.tag.gettags(client.focus.screen)[i]
                          if tag then
...
--try to execute all autostart programs
...
--Volume hook
pbvolume_timer = timer ({timeout = 1})
pbvolume_timer:connect_signal("timeout", function()
  volume("update", pb_volume)
end)
pbvolume_timer:start()

-- Naughty preferences
-- Show pop-ups forever
